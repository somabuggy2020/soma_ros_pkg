...
public:
                 ...
                 pnh(ros::NodeHandle("~"))
    {
        ...
        odom_topic = pnh.param<sstring>("odom_topic", "/odom");
        ...
        //make states
        sStop = new Stop();
        sMoveTo = new MoveTo(3.5);
        sHome = new Home(2.5);
        states[State::Stop] = sStop;
        states[State::MoveTo] = sMoveTo;
        states[State::Home] = sHome;
...
private:
    void main(const ros::TimerEvent &e)
    {
        ...
        //command send to other CPU via ethernet
        comm->send(data->Uin.lambda, data->Uin.v);
        ...
        jsktxt.text += "STEER:" + std::to_string(RAD2DEG(data->Uin.lambda)) + "\n";
        jsktxt.text += "V ROT:" + std::to_string(data->Uin.v) + "\n";
        txt_pub.publish(jsktxt);

        std_msgs::Float32MultiArray famsgs;
        famsgs.data.resize(2);
        famsgs.data[0] = data->Uin.lambda;
        // famsgs.data[1] = data->vt_rot;
